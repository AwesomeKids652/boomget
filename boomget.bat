...
:totalprompt
cls
title BoomGet Prompt
echo !ORIGINAL PROJECT BY KVC!
echo Welcome to boomget!
echo Anytime you want to go back to WinBoomOS
echo Just type "winboom"
echo boom help for commands
...
:ResetAll
REM Removing BoomGet from Path...
Call Getlen "!_path!"
Set _len=!Errorlevel!
Set _NewPath=
REM Reading Path of Current User...
...
Set /p ".=Removing BoomGet From PATH... " <nul
...
REM Checking for BoomGet Update...
Set _UpdateBat=
Set _online_ver=
wget -qO- "https://raw.githubusercontent.com/Bat.bat" | find /i "Set _ver=" > "!Temp!\_Ver.txt"
...
If !_online_ver! GTR !_ver! (
	Echo. --------------------------------------------------------------------------
    Echo. A NEW VERSION OF BOOMGET IS AVAILABLE... [Current: !_Temp_ver!, New: !_Temp_online_ver!]
	Echo. --------------------------------------------------------------------------
	Echo.
    Wget "https://github.com/AwesomeKids652/boomget/archive/main.zip" -O "Zips\boomget.zip" -q --tries=5 --show-progress --timeout=5
    	REM Creating a separate batch-file, as script overwriting  itself can lead to malfunctioning...
   	(
	Echo @Echo off
	Echo SetLocal EnableDelayedExpansion
    Echo Title Updating BoomGet...
    Echo cls
    Echo Echo. Extracting... to 'PATH'
    Echo Pushd files
    Echo 7za e -y "!_path!\Zips\boomget.zip"
	...
	Echo Del /f /q "!_path!\Zips\boomget.zip" 
	Echo Pause
	) >"!Temp!\Updateget.bat"
	Set _Updateget=True
)
...
Echo. CREDITS: Bat By KVC Modded for winboomos by awesomekid
echo.
